...
int main(int argc, char **argv){
    ...
    size_t chromosomePopulationSize = 20; //Size o49f the population
    size_t chromosomeLength = 10; //Length of each chromosome
    double percentCrossOver = 0.7; //Percent of population used for crossover
    ...
    while (!population.checkForGoal()) {
        vector<Chromosome> generation_offspring;
        ...
        for (int i = 0; i < crossOverTotal / 2; i++) {
            ...
            vector<Chromosome> offspring = GeneticAlgorithm::single_crossover(population.selectFrontParent(),
                                                                              ...
                                                                              bitmask);
            //Add the two new offspring to the generation's offspring
            for (Chromosome c : offspring) {
                generation_offspring.push_back(c);
            }
        }
        //Insert the generation's offspring into the population
        population.insertOffspring(generation_offspring);
        g++;
